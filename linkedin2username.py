...
class NameMutator():
    ...
    def split_name(name):
        ...
        """
        parsed = [p for p in re.split(' |-', name) if p]

        if not parsed:
            split_name = {'first': '', 'second': '', 'last': ''}
        elif len(parsed) == 1:
            split_name = {'first': parsed[0], 'second': '', 'last': ''}
        elif len(parsed) > 2:
    ...
    def f_last(self):
        """jsmith"""
        names = set()
        if not self.name['first'] or not self.name['last']:
            return names
...
def login():
    ...
    li_user = os.environ.get('LI_USER', '')
    li_pass = os.environ.get('LI_PASS', '')
    if li_user and li_pass:
        print("[*] Identifiants détectés dans les variables d'environnement, remplissage auto...")
        time.sleep(2)  # attendre le chargement de la page
        try:
            driver.find_element("id", "username").send_keys(li_user)
            driver.find_element("id", "password").send_keys(li_pass)
            driver.find_element("css selector", "button[type='submit']").click()
        except Exception:
            print("[!] Remplissage auto échoué, connectez-vous manuellement.")
    else:
        print("[*] Astuce : définissez LI_USER et LI_PASS pour le remplissage automatique.")
